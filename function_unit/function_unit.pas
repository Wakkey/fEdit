...
uses
  ...
  {$IFDEF WIN32}, shellAPI, WIndows {$ENDIF};
...
begin
  ...
  try
    ...
    mainform.AllowDropFiles := not true;
  ...
  for I := 0 to ParamCount do begin
      //functionunit.Run(functionunit.EnumFileFromDir(ansitoutf8(ParamStr(i)),st));
      functionunit.EnumFileFromDir(ansitoutf8(ParamStr(i)),st)
  end;
  ...
  functionunit.toolbarbuttonparnt2.Clear;
   ...
   if -1 < i then begin
     for i1 := 0 to strtoint(st[i+1]) -1 do begin
       ...
       {$IFDEF WIN32}if not true_boot then begin{$ENDIF}
         inc(tabtoolbarcount);
       {$IFDEF WIN32}end;{$ENDIF}
...
var
 url:string;
 {$IFDEF WIN32}ShellEx: TShellExecuteInfo;{$ENDIF}
begin
  {$IFDEF WIN32}
    url := utf8toansi(file_name);
...
//フォルダ再帰検索ファイル一覧取得関数
function Tfunctionunit.EnumFileFromDir(Dir: string;st:TstringList):boolean;
var
  Rec: TSearchRec;
