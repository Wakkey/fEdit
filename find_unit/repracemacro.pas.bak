...
begin
  if s = '先頭' then begin
    //functionunit.Run(functionunit.reprace_macro(s,find,reprace,count));
    functionunit.reprace_macro(s,find,reprace,count)
  end;
  if s = '中間' then begin
    //functionunit.Run(functionunit.reprace_macro(s,find,reprace,count));
    functionunit.reprace_macro(s,find,reprace,count)
...
begin
  ...
  //mainform.pageControl1.PageIndex := strtoint(s1);
