...
type
 ...
 private
  ...
  public
    ...
    count_tec_comp,count_prt:integer;
...
begin
  ...
  for i := 0 to FCFM.comp.count - 1 do begin
    ...
    FCFM.meisi_base.Canvas.pen.Color := clblack;//strtoint(st[3]);
    //FCFM.meisi_base.Canvas.brush.Style := clblue;
    ...
    if FCFM.comp.Items[i].Tag = 2 then begin
      with FCFM.bmp_base.Canvas do begin
       ...
       showmessage(utf8toansi(set_name));
        ...
        //font.Name := st[1];
        font.Size := 15 * bx2;
        pen.Color := clblack;//strtoint(st[3]);
        brush.Style := bsClear;
        i2 :=0;
    ...
    end; if FCFM.comp.Items[i].Tag = 1 then begin
  ...
  FCFM.meisi_base.SaveToFile(fcfm.setprjdir + 'temp.jpg');
end;
...
var
  s2:string;
  function set_img_no(i:integer):string;
  var
    s1:string;
  begin
    s1:='';
...
begin
  if s = 'f' then begin
    ...
    s2 := set_img_no(FCFM.ary_TFC_Comp[fcfm.count_tec_comp-1].img_No);
    FCFM.ComboBox6.Items.Add(inttostr(FCFM.comp.count div 3) + s + s2);
    FCFM.ComboBox1.Items.Add(inttostr(FCFM.comp.count div 3) + s + s2);
    FCFM.ComboBox9.Items.Add(inttostr(FCFM.comp.count div 3) + s + s2);
  ...
  while i < Count  do begin
    ...
    if SL[i+3] = '1' then begin
      try
        ...
        FCFM.ComboBox6.Items.Add( inttostr((i1 div 3)+1) + 'f');
        FCFM.ComboBox1.Items.Add( inttostr((i1 div 3)+1) + 'f');
        FCFM.ComboBox9.Items.Add( inttostr((i1 div 3)+1) + 'f');
      except end;
      cmpname := 'f';
...
function TFcfm.write_canvas(i:integer):boolean;
var
  i1,i2,img_No,ct,cl,cw,ch,ch0,cw0,cw1:integer;
  st:TStringList;
  J:TJPegImage;
begin
  try
  ...
  //if loadPIC then begin
    try
        ...
        meisi_base:= Timage.Create(pnl);
        meisi_base.Parent := pnl;
        meisi_base.Name:= 'i' + inttostr(i+1);
        meisi_base.Align:= alleft;
        pnl.img_No:=0;
        meisi_base.Width:= 60;
        meisi_base.Canvas.Brush.Color:= clWhite;
        meisi_base.Canvas.Clear;
        meisi_base.Canvas.Rectangle(0,0,meisi_base.Width,meisi_base.Height);
        meisi_base.Canvas.Pen.Color:= clblack;
        meisi_base.Canvas.Pen.Width:=2;
        meisi_base.Canvas.TextOut(cl + 2,ct + 2,'処理');
        meisi_base.Canvas.Rectangle(5,ch div 2,45,(ch div 2)+20);
        meisi_base.OnClick:= FCFM.Image1.OnClick;
        meisi_base.OndblClick:= FCFM.Image1.OndblClick;
        FCFM.comp.Add(meisi_base);
        ...
        try
          try
           st := TStringList.Create;
           st.LoadFromFile(setprjdir + inttostr(i)+ '_No');
           img_No:= strtoint(st[0]);
           st.Free;
          except
            img_No := 0;
          end;

         ct := ary_TFC_Comp[i].top;
         cl := ary_TFC_Comp[i].left;
         cw := ary_TFC_Comp[i].TIM.Left + ary_TFC_Comp[i].TIM.width;
         cw0 := ary_TFC_Comp[i].left + ary_TFC_Comp[i].width;
         cw1 := ary_TFC_Comp[i].left + ary_TFC_Comp[i].TIM.Width;
         ch0 := ary_TFC_Comp[i].top + ary_TFC_Comp[i].height;
         ch := ary_TFC_Comp[i].TIM.Top + ary_TFC_Comp[i].TIM.height;



         //FCFM.Image1DblClick( meisi_base );
            meisi_base.Canvas.Brush.Color:= clWhite;
            //meisi_base.Canvas.Clear;
            //meisi_base.Canvas.Rectangle(cl,ct,cw,ch0);
            meisi_base.Canvas.Rectangle(cl,ct,cw0,ch0);
            //meisi_base.Canvas.Rectangle(cl,ct,ary_TFC_Comp[i].TIM.Width,ch0);
            meisi_base.Canvas.Pen.Color:= clblack;
            meisi_base.Canvas.Pen.Width:=2;
         ...
         try
             st := TStringList.Create;
             i2 := 0;
             st.LoadFromFile( setprjdir + inttostr(i) + 'm');
             for I1 := 0 to st.Count - 1 do begin
               meisi_base.Canvas.TextOut(cw1,ct+ i2+2 ,st[i1]);
               i2 := i2 + meisi_base.Canvas.TextHeight(st[i1]);
             end;
             st.Free;
          except
            ...
            case img_No of
               0: begin
                  meisi_base.Canvas.Rectangle(cl + 5,(ch0 -(ch div 2)),cl + 45,(ch0 -(ch div 2))+20);
                  meisi_base.Canvas.TextOut(cl + 2,ct + 2,'処理');
               end;
               1: begin
                    meisi_base.Canvas.TextOut(cl + 2,ct + 2,'判断');
                    with meisi_base.Canvas do begin
                         MoveTo(cl+15,(ch0 -(ch div 2))+5);
                         LineTo(cl+5,(ch0  -(ch div 2))+15);
                         LineTo(cl+15,(ch0  -(ch div 2))+25);
                         LineTo(cl+25,(ch0  -(ch div 2))+15);
                         LineTo(cl+15,(ch0  -(ch div 2))+5);
                   end;
               end;
               2: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'定義済');
                   meisi_base.Canvas.TextOut(cl + 2,ct +20,'処理');
                   meisi_base.Canvas.Rectangle(cl +5,(ch0 -(ch div 2)),cl +45,(ch0 -(ch div 2))+20);
                   meisi_base.Canvas.Rectangle(cl +15,(ch0 -(ch div 2)),cl +35,(ch0 -(ch div 2))+20);
               end;
               3: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'手作業');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+5,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+5);
                       LineTo(cl+30,(ch0 -(ch div 2))+15);
                       LineTo(cl+10,(ch0 -(ch div 2))+15);
                       LineTo(cl+5,(ch0 -(ch div 2))+5);
                   end;
               end;
               4: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'組合わせ');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+5,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+5);
                       LineTo(cl+20,(ch0 -(ch div 2))+15);
                       LineTo(cl+20,(ch0 -(ch div 2))+15);
                       LineTo(cl+5,(ch0 -(ch div 2))+5);
                   end;
               end;
               5: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'抜出');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+25,(ch0 -(ch div 2))+5);
                       LineTo(cl+25,(ch0 -(ch div 2))+5);
                       LineTo(cl+40,(ch0 -(ch div 2))+15);
                       LineTo(cl+10,(ch0 -(ch div 2))+15);
                       LineTo(cl+25,(ch0 -(ch div 2))+5);
                   end;
               end;
               6: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'照合');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+5,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+5);
                       LineTo(cl+20,(ch0 -(ch div 2))+15);
                       ...
                       MoveTo(cl+20,(ch0 -(ch div 2))+15);
                       LineTo(cl+20,(ch0 -(ch div 2))+15);
                       LineTo(cl+35,(ch0 -(ch div 2))+25);
                       LineTo(cl+5,(ch0 -(ch div 2))+25);
                       LineTo(cl+20,(ch0 -(ch div 2))+15);
                   end;
               end;
              7: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'分類');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+20,(ch0 -(ch div 2))+5);
                       LineTo(cl+20,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+15);
                       LineTo(cl+5,(ch0 -(ch div 2))+15);
                       LineTo(cl+20,(ch0 -(ch div 2))+5);

                       MoveTo(cl+5,(ch0 -(ch div 2))+15);
                       LineTo(cl+35,(ch0 -(ch div 2))+15);
                       LineTo(cl+20,(ch0 -(ch div 2))+25);
                       LineTo(cl+20,(ch0 -(ch div 2))+25);
                       LineTo(cl+5,(ch0 -(ch div 2))+15);
                   end;
               end;
               8: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'手操作');
                   meisi_base.Canvas.TextOut(cl + 2,ct +20,'入力');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+5,(ch0 -(ch div 2))+8);
                       LineTo(cl+35,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+15);
                       LineTo(cl+5,(ch0 -(ch div 2))+15);
                       LineTo(cl+5,(ch0 -(ch div 2))+8);
                   end;
               end;
               9: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'入出力');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+10,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+5);
                       LineTo(cl+30,(ch0 -(ch div 2))+15);
                       LineTo(cl+5,(ch0 -(ch div 2))+15);
                       LineTo(cl+10,(ch0 -(ch div 2))+5);
                   end;
               end;
              10: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'オンライン');
                   meisi_base.Canvas.TextOut(cl + 2,ct +20,'記録');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+15,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+5);
                       LineTo(cl+30,(ch0 -(ch div 2))+10);
                       LineTo(cl+35,(ch0 -(ch div 2))+15);
                       LineTo(cl+15,(ch0 -(ch div 2))+15);
                       LineTo(cl+10,(ch0 -(ch div 2))+10);
                       LineTo(cl+15,(ch0 -(ch div 2))+5);
                   end;
               end;
              11: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'書類');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+10,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+10);
                       LineTo(cl+30,(ch0 -(ch div 2))+15);
                       LineTo(cl+10,(ch0 -(ch div 2))+15);
                       LineTo(cl+10,(ch0 -(ch div 2))+5);
                   end;
              ...
              12: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'紙カード');
                   with meisi_base.Canvas do begin
                       MoveTo(cl+15,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+5);
                       LineTo(cl+35,(ch0 -(ch div 2))+15);
              ...
              13: begin
                   meisi_base.Canvas.TextOut(cl + 2,ct + 2,'磁気ディスク');
                   with meisi_base.Canvas do begin
                       meisi_base.Canvas.Brush.Color:= clblack;
                       //Arc(5, -(ch0 -(ch div 2)), 50, -(ch0 -(ch div 2))+10,300,10,300,20);
                       Ellipse(cl+5, (ch0 -(ch div 2))+10,cl + 50, (ch0 -(ch div 2))+20);
                       Ellipse(cl+5, (ch0 -(ch div 2))+5, cl +50, (ch0 -(ch div 2))+15);
                       meisi_base.Canvas.Brush.Color:= clwhite;
                       Ellipse(cl+5, (ch0 -(ch div 2)),cl + 50, (ch0 -(ch div 2))+10);
                   end;
               end;
            end;
        except
    ...
    try
      //showmessage(setprjdir + 'pic.bmp');
      //meisi_base.SaveToFile( utf8tosys(setprjdir + 'pic.bmp') );
...
function TFCFM.write_line(i:integer):boolean;
var
  i1,i2:integer;
  m:TMemo;
begin
    ...
    try
      //if 1 < length(FCFM.comp.Items[i].Hint) then begin
       ...
       //caption := FCFM.comp.Items[i].Hint;
        ...
        //showmessage((fcfm.setprjdir+ inttostr(i)));
        ...
        if -1 < i1 then begin
          ...
          if 0 < ansipos('青矢印',' ' + m.Lines[0]) then begin
            meisi_base.Canvas.Pen.Color:= clblue;
          end else if 0 < ansipos('赤矢印',' ' + m.Lines[0]) then begin
            meisi_base.Canvas.Pen.Color:= clred;
          end;
          meisi_base.Canvas.Pen.Width:= 4;
          meisi_base.Canvas.MoveTo(FCFM.ary_TFC_Comp[i1].Left + (FCFM.ary_TFC_Comp[i1].Width div 2),
          ...
          if -1 < i2 then begin
            ...
            meisi_base.Canvas.LineTo(FCFM.ary_TFC_Comp[i2].Left + (FCFM.ary_TFC_Comp[i2].Width div 2),
...
begin
  ...
  if 0 < ansipos('f',combobox6.Text) then begin
    try
...
begin
  ...
  end else if 0 < ansipos('f',combobox6.Text) then begin
    try
      //set_editcomp(combobox6.itemIndex,memo1);
         ...
         // showmessage(FCFM.comp[i].Name + char(13)+ inttostr(i));
          ...
          if FCFM.comp[i].Name = 'i' + inttostr(i) then begin
            ...
            case ary_TFC_Comp[i1].img_No of
               ...
               4: begin
                   ary_TFC_Comp[i1].TIM.Canvas.TextOut(2,2,'組合せ');
                   with ary_TFC_Comp[i1].TIM.Canvas do begin
                       MoveTo(5,(ary_TFC_Comp[i1].TIM.Height div 2)+5);
                       LineTo(35,(ary_TFC_Comp[i1].TIM.Height div 2)+5);
...
procedure TFCFM.Button4Click(Sender: TObject);
    var
      ...
      Document   : Variant;
      TextCursor : Variant;
      sl,set_s1 :TStringList;
      url,s,s1 :string;
      i,i1:integer;
      J:TJPegImage;
      B:TBitmap;
    const
      WordServerName = 'Word.Application';
      ExcelServerName = 'Excel.Application';
      //function load_office(s:widestring):boolean;
        function run_office(s:widestring):boolean;
        begin
          //if Assigned(InitProc) then
            //TProcedure(InitProc);
          try
            //Server := CreateOleObject(s);
          except
            showmessage('Unable to start '+ s + '.');
            Exit;
          end;
        end;
...
begin
   if not savedialog1.Execute then
     exit;
    //FCFM.MeisiForm.Visible:=false;
      //FCFM.print();
             meisi_base := TBitmap.Create;
             meisi_base.Canvas.Brush.Color := clBlack;
             meisi_base.Width := FCFM.MeisiPIc.Width;
             meisi_base.Height := FCFM.MeisiPIc.Height;
             meisi_base.Canvas.Create;
             meisi_base.Canvas.Brush.Color:= clWhite;
             meisi_base.Canvas.Rectangle(0,0,meisi_base.Width,meisi_base.Height);
             meisi_base.Canvas.Brush.Color := clwhite;
             meisi_base.Canvas.pen.Color := clBlack;
  ...
  sl := TStringList.Create;;
  SL.LoadFromFile(utf8toansi(ansitoutf8(ExtractFilePath( Paramstr(0) )) + listbox1.Items[listbox1.itemindex] + dir +'ForwCreate.csv' ));
  count := strtoint(sl[2]) +1;
  i := 0;
  i1 := 0;
  while i < Count do begin
    if SL[i+3] <> '1' then begin
      //showmessage(sl[i+3] +','+ inttostr(i)+','+ inttostr(i1));
      write_line(i1);
      inc(i1);
    end;
    i := i + 6;
  ...
  {$IFDEF Windows}
    J.SaveToFile(utf8tosys(savedialog1.FileName));
  {$ENDIF}
  {$IFDEF LINUX}
  ...
  {$ENDIF}
      ...
      try
        try
          ...
          showmessage(ansitoutf8(ExtractFilePath( Paramstr(0) ) + 'temp.jpg' + ' not open'));
        end;
      finally
        //B.Free;
        //J.Free;
